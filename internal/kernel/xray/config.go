...
// Recommended xray-core policy defaults for levels.0. These mirror a
// production-friendly profile: aggressive handshake/idle timeouts so stale
// connections are recycled quickly, and a bounded per-connection buffer.
const (
	defaultXrayHandshake    = 4
	defaultXrayConnIdle     = 35
	defaultXrayUplinkOnly   = 3
	defaultXrayDownlinkOnly = 5
	defaultXrayBufferSizeKB = 128
)

// buildXrayPolicy generates the xray-core policy block from KernelConfig.
// Timeout fields <= 0 fall back to the recommended defaults. BufferSizeKB:
// 0 = default, -1 = unlimited (passed through), N = N KB.
func buildXrayPolicy(kcfg config.KernelConfig) M {
	p := kcfg.XrayPolicy

	handshake := p.Handshake
	if handshake <= 0 {
		handshake = defaultXrayHandshake
	}
	connIdle := p.ConnIdle
	if connIdle <= 0 {
		connIdle = defaultXrayConnIdle
	}
	uplinkOnly := p.UplinkOnly
	if uplinkOnly <= 0 {
		uplinkOnly = defaultXrayUplinkOnly
	}
	downlinkOnly := p.DownlinkOnly
	if downlinkOnly <= 0 {
		downlinkOnly = defaultXrayDownlinkOnly
	}
	bufferSize := p.BufferSizeKB
	if bufferSize == 0 {
		bufferSize = defaultXrayBufferSizeKB
	}
...
func mergeCustomXray(cfg M, kcfg config.KernelConfig) {
	...
	// policy — custom deep-merges into the generated levels/system
	if v, ok := custom["policy"]; ok {
		mergeCustomXrayPolicy(cfg, v)
	}
...
// mergeCustomXrayPolicy deep-merges a custom xray policy into the generated
// one. Custom values win per-field; stats flags and system stats generated by
// xboard-node are preserved unless the custom config overrides them.
func mergeCustomXrayPolicy(cfg M, customPolicy any) {
	customMap, ok := customPolicy.(map[string]any)
	if !ok {
		return
	}
	...
	if lv, ok := customMap["levels"].(map[string]any); ok {
		curLevels, _ := cur["levels"].(M)
		if curLevels == nil {
			curLevels = M{}
		}
		for lvl, v := range lv {
			lvlMap, ok := v.(map[string]any)
			if !ok {
				continue
			}
			existing, _ := curLevels[lvl].(M)
			if existing == nil {
				existing = M{}
			}
			for k, vv := range lvlMap {
				existing[k] = normalizePolicyValue(vv)
			}
			curLevels[lvl] = existing
		}
		cur["levels"] = curLevels
	}

	if sys, ok := customMap["system"].(map[string]any); ok {
		curSys, _ := cur["system"].(M)
		if curSys == nil {
			curSys = M{}
		}
		for k, v := range sys {
			curSys[k] = normalizePolicyValue(v)
		}
		cur["system"] = curSys
	}
...
// normalizePolicyValue converts JSON floats with integral values to ints so
// xray's uint32/int32 policy fields parse cleanly.
